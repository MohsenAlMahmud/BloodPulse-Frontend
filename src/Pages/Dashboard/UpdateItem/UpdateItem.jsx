...
const UpdateItem = () => {
    ...
    const onSubmit = async(data) => {
        ...
        if(res.data.success){
            //send the item data to the server with the image
            const menuItem = {
                name: data.name,
            ...
            <div>
                <form onSubmit={handleSubmit(onSubmit)}>
                    ...
                    <div className="form-control">
                        ...
                        <textarea className="textarea textarea-bordered h-24" defaultValue={recipe} {...register('recipe')} placeholder="Recipe Details"></textarea>
                    </div>
                    <div className="form-control w-full my-6">
                        <input type="file" {...register('image', {required: true})} className="file-input w-full max-w-xs" />
                    </div>

                    <button className="btn">Update Menu Item<FaUtensils></FaUtensils></button>
                </form>
            </div>
        </div>
    );
